function showAdminStudentPanel() {
	console.log(' in Sudent admin click ... ');
	var amp = $('#adminMainPanel');
	amp.empty();
	
	var sasdp = $('<div id="studentAdminSearchDockPanel" style="height:550;"/>');
	amp.append(sasdp);

	var sacip = $('<div id="studentAdminClassInputPanel" style="border:0px solid;"/>');
	sasdp.append(sacip);
	
	var sddldiv = $('<div style="margin-top:10px; border:1px solid  #e0e9f5; width:200px;"/>');
	var cddldiv = $('<div style="margin-top:10px; border:1px solid  #e0e9f5; width:240px;"/>');
	var btndiv = $('<div style="margin-top:10px; border:1px solid  #e0e9f5;"/>');
	sacip.append(sddldiv);
	sacip.append(cddldiv);
	sacip.append(btndiv);
	
	sddldiv.append('<label style="float:left; margin-top:3px; margin-left:5px">Semester : </label>');
	var sname = $('<div style="float:left; margin-left:5px"/>').attr({id:'ddlSemesterSearchNameInAdminStudent'});
	sddldiv.append(sname);
	sname.jqxDropDownList({placeHolder: "Semester", height: 20, width: 120, dropDownHeight: 120, theme: getTheme()});
	var activesource = {
		datafields:[
			{ name: 'id',   type: 'int'}, 
	...
	cddldiv.append('<label style="float:left; margin-top: 3px; margin-left:5px">Class : </label>');
	var ddlcname = $('<div style="float:left; margin-left:5px"/>').attr({id:'ddlStudentAdminClassSearchName'});
	cddldiv.append(ddlcname);
	var allsource = {
		datafields:[
			{ name: 'value',   type: 'int'}, 
	...
	ddlcname.jqxDropDownList({placeHolder: "Please Select Class Name", height: 20, width: 190, dropDownHeight:190, theme: getTheme(), selectedIndex:-1, source:alldataadapter, displayMember: "text", valueMember: "value"});
	...
	cbutton.jqxButton({ width: '75', height: 20, theme: getTheme() });
	...
	sbutton.jqxButton({ width: '75', height: 20, theme: getTheme() });

	sacip.jqxDockPanel({height: 40});
	sddldiv.attr('dock', 'left');
	cddldiv.attr('dock', 'left');
	btndiv.attr('dock', 'left');

	var sagd = $('<div id="studentAdminGridDiv" style="border:0px solid;"/>');
	amp.append(sagd);
	
	addAdminStudentEventListerners();
}

function addAdminStudentEventListerners() {
	...
	$('#ddlSemesterSearchNameInAdminStudent').on('change', function(event) {
	    var args = event.args;
    	if (args) {
	    	var item = args.item;
			...
			$('#ddlStudentAdminClassSearchName').jqxDropDownList({selectedIndex: -1});
...
function loadAdminStudentTabSemesterDropDownListDataSource(data) {
	var activesource = {
		datafields:[
			{ name: 'id',   type: 'int'}, 
			{ name: 'name',  type: 'string'}
	...
	$('#ddlSemesterSearchNameInAdminStudent').jqxDropDownList({source:activedataadapter, displayMember: "name", valueMember: "id"});
	
	var sid = getCurrentSemester().id;
	
	$('#ddlSemesterSearchNameInAdminStudent').jqxDropDownList('selectItem',sid);
} 
